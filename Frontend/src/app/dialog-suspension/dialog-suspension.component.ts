...
@Component({
  selector: 'app-dialog-suspension',
  templateUrl: './dialog-suspension.component.html',
  styleUrls: ['./dialog-suspension.component.css']
})
export class DialogSuspensionComponent implements OnInit {
  motif = new FormControl('', [Validators.minLength(10), Validators.required]);

  constructor(public dialogRef: MatDialogRef<DialogSuspensionComponent>, @Inject(MAT_DIALOG_DATA) public data: any) { }
  ...
  getErrorMessage() {
    ...
    if(this.motif.hasError('required'))
      return 'Vous devez donner un motif de suspension'
  }
